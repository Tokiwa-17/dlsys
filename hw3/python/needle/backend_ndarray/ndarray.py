...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        if self.is_compact() == False: raise ValueError('current shape is not equal to the product of the new shape')
        stride = 1
        res = []
        for i in range(1, len(new_shape) + 1):
            res.append(stride)
            stride *= new_shape[-i]
        return self.make(new_shape, tuple(res[::-1]), self._device, self._handle, self._offset)
        ## END YOUR SOLUTION
